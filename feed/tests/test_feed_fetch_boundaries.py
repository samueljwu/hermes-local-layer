...
def test_feed_ops_rejects_private_url_before_network(monkeypatch):
    ...
    monkeypatch.setattr(feed_ops.urllib.request, 'urlopen', fail_urlopen)
...
def test_extractor_rejects_private_redirect(monkeypatch):
    ...
    class Response:
        ...
        def read(self, *args, **kwargs):  # pragma: no cover - rejected before read
            raise AssertionError('redirect target should be checked before read')

    monkeypatch.setattr(extractors.urllib.request, 'urlopen', lambda *args, **kwargs: Response())
