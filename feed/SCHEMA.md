...
Network fetches for source discovery, RSS/API reads, and article-page fallback must fail closed before network I/O for non-`http(s)` URLs and any host that does not resolve exclusively to globally routable public IPs, including localhost, private, link-local, metadata, reserved, and CGNAT ranges. Fetch helpers must re-check the final URL after redirects and cap response size. The static `/feed/` publication exception is exact-path only: the renderer may write `/home/hermes/homepage/dist/feed/index.html` and must reject symlinked output directories/files or arbitrary alternate output paths.
