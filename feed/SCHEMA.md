...
Digest runs snapshot protected systems before and after generation using content hashes, not mtimes, so mtime-only rewrites do not fail the run. The snapshot intentionally excludes regenerated wiki semantic build artifacts under `/home/hermes/wiki/src/_meta/semantic/` except durable curation files such as `accepted-edges.json` and `rejected-edges.json`; wiki builds can rewrite the generated graph/lint/state files with timestamp churn while the feed is running, and those derived files are not canonical wiki knowledge. Digest text, replacement history, and page HTML are staged in memory; the run file, recommendation history, public page, and log are promoted only after the protected comparison passes.
...
Network fetches for source discovery, RSS/API reads, and article-page fallback must fail closed before network I/O for non-`http(s)` URLs and any host that does not resolve exclusively to globally routable public IPs, including localhost, private, link-local, metadata, reserved, and CGNAT ranges. Fetch helpers must connect to a prevalidated IP address while preserving the original Host/SNI, re-check every redirect/final URL, and cap response size. A Bing News redirect wrapper from a bounded fallback is accepted only when its explicit `http(s)` target matches the exact destination host and path boundary encoded in the feed URL's `site:` query; off-domain, sibling-prefix, literal or encoded dot-segment, and backslash traversal paths are rejected. The static `/feed/` publication exception is exact-path only: the renderer may write `/home/hermes/homepage/dist/feed/index.html` and must reject symlinked output ancestors/directories/files or arbitrary alternate output paths before creating or writing anything.
...
- Slots 1-3: genuinely interest-related (`relevance >= 0.8`)
...
- The fixed digest shape remains authoritative: exactly three interest-related items and exactly two exploratory items. If fewer than three candidates meet the Core Pick relevance threshold, selection fails explicitly rather than forcing zero-relevance candidates into Core Picks under an `adjacent_interest` label.
...
### Structural validation and safe publication

`validate` checks these contracts without writing defaults, fetching sources, or changing rankings. Diagnostics identify the file and malformed field. `_tools/feed_state_schema.py` also checks proposed canonical JSON before `save_json` publishes it. Digest profile, fetch state/cache, history and rendered outputs are staged in memory; schema validation and the protected snapshot gate precede all their writes. Standalone page/report generation validates its canonical input before publishing. Fetch inspection with `--no-save` does not persist a fallback profile.

- **Profile:** object with `active_interests` as a list of objects, each with a nonempty string `topic` and optional finite numeric `weight` (absence retains the consumer's 0.5 default; booleans and numeric strings are not numbers). Present keyword/query/evidence fields are string lists; legacy `terms` is a string list or null. Optional policy is an object and notes/negative signals are lists.
- **History:** list of recommendation objects with string candidate/source/title/run/date/relation fields, integer slot 1–5, known relation type, and an HTTP(S) article or HN URL. Present scores must be finite numbers; nullable historical text and category fields remain supported. Older records need not acquire `pick_id`, summary, or correlation fields. Validation does not rewrite or re-rank historical picks.
- **Source state:** object mapping source IDs to objects with optional string-list `seen_ids` (absence retains the empty-list default) and optional string/null `last_checked`. Reserved `last_fetch_errors` is a list of objects with nonempty string source, string error (including empty exception messages), and optional string/null check timestamps/title.
- **Source registry:** object with an `allowed_candidate_sources` list of objects containing nonempty names and HTTP(S) endpoints. Present IDs must be unique; present `enabled` values must be booleans, topic lists must contain strings, and semantic roles must be recognized. Legacy bootstrap rows may omit id/connector/enabled. Optional read-only source records and forbidden-rule lists have their own structural checks. Connector quality and source-policy lint remain separate from offline state validation.

Source purpose, connector/kind and role metadata may be absent, empty or null, preserving existing warning/fallback behavior. Nonempty roles are checked case-insensitively after trimming, as in the consumer; the stored spelling is not rewritten. Expected topics may be null or a string list. Wrong types and unknown nonempty roles remain errors. Unknown extension metadata is preserved, not stripped. These contracts are structural rather than a historical-data migration or network/semantic quality gate.

`_tools/feed_safe_io.py` is a feed-local helper shared by the harness and page renderer, using the source-pin updater's descriptor traversal pattern without importing its Discord/network code. Every absolute parent component is opened with `O_DIRECTORY | O_NOFOLLOW`; missing directories are created relative to an already verified parent. Temporary creation, file fsync, atomic replacement, cleanup, append, directory fsync, and lock creation use verified directory descriptors rather than reopening pathnames. Lock/append inodes must be regular files, and symlink/non-regular publication destinations are rejected. Parent renames after binding cannot redirect publication or cleanup to a replacement symlink. The homepage exception remains the exact canonical `index.html` path. Each replacement is atomic, but the sequence of multiple digest outputs is not a filesystem transaction.
