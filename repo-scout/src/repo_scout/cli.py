...
from .secure_fs import SecureDirectory, open_output_directory, require_lexically_under
...
def _require_under(path: Path, root: Path, label: str) -> Path:
    return require_lexically_under(path, root, label)
...
def _scout_lock(out_dir: Path):
    with open_output_directory(DEFAULT_OUT_DIR, out_dir) as (root_dir, selected_dir):
        with root_dir.lock():
            yield selected_dir


def _write_json(path: Path, data: object, directory: SecureDirectory | None = None) -> None:
    if directory is None:
        with open_output_directory(DEFAULT_OUT_DIR, path.parent) as (_, opened):
            opened.atomic_write_json(path.name, data)
        return
    directory.atomic_write_json(path.name, data)


def _error_result(error: GitHubAPIError, cfg: ScoutConfig, out_dir: Path, started_at: datetime, counts: dict | None = None, *, secure_dir: SecureDirectory | None = None) -> dict:
    result = {
    ...
    _write_json(out_dir / "error_report.json", result, secure_dir)
    return result
...
def run_scout(config_path: Path, out_dir: Path, dry_run: bool = False, feedback_path: Path | None = None) -> dict:
    ...
    with _scout_lock(out_dir) as secure_dir:
        return _run_scout_locked(config_path, out_dir, dry_run=dry_run, feedback_path=feedback_path, secure_dir=secure_dir)


def _run_scout_locked(config_path: Path, out_dir: Path, dry_run: bool = False, feedback_path: Path | None = None, *, secure_dir: SecureDirectory | None = None) -> dict:
    ...
    try:
        client = GitHubClient(
            ...
            cache_directory=cache_secure_dir,
        )
        ...
        if dry_run:
            ...
            _write_json(out_dir / "dry_run.json", result, secure_dir)
            return result
        ...
        try:
            ...
            return _error_result(error, cfg, out_dir, now, counts, secure_dir=secure_dir)
        ...
        _write_json(out_dir / "shortlist.json", result, secure_dir)
        return result
    finally:
        if cache_secure_dir is not None:
            cache_secure_dir.close()
