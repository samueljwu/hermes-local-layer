...
def test_manual_chart_joins_name_only_registry_by_stable_suffix(tmp_path, monkeypatch, capsys):
    ...
    registry.write_text(json.dumps([
        {'id': 'T-1-7', 'name': 'Renamed active task', 'tag': 'School', 'status': 'in_progress'},
        {'id': 'T8', 'name': 'Current closed title', 'tag': 'Admin', 'status': 'completed'},
    ]))
    ...
    before = (registry.read_bytes(), log.read_bytes())
    ...
    assert before == (registry.read_bytes(), log.read_bytes())
